...
def register_routes(app):
    ...
    def _render_url_result(ip="Unknown", url_result_id=None, scan_id=None):
        ...
        if current_scan_id:
            ...
            # Fallback ONLY for legacy invocations without a scan_id (e.g. /url-result/<ip>)
            ports = []
            if ip and ip != "Unknown":
                ports = conn.execute(
                    """
        ...
        try:
            ...
            if url_intel_row:
                ...
                dns_loaded = False
                if raw_url_info.get("dns_records"):
                    try:
                        parsed_dns = json.loads(raw_url_info["dns_records"])
                        if isinstance(parsed_dns, dict) and any(parsed_dns.values()):
                            raw_url_info["dns"] = parsed_dns
                            dns_loaded = True
                    except Exception:
                        pass
                if not dns_loaded:
            ...
            logger.error(f"URL Intelligence Error: {e}")
            raw_url_info = {}
    ...
    def view_vulnerabilities():
        ...
        if scan_id and not target:
            ...
            if not row_target:
                row_target = conn.execute(
                    "SELECT ip AS target_ip FROM vulnerabilities WHERE scan_id=? LIMIT 1", (scan_id,)
                ).fetchone()
            if row_target and row_target["target_ip"]:
                target = row_target["target_ip"]
    ...
    def view_cves():
        ...
        if scan_id and not target:
            ...
            if not row_target:
                row_target = conn.execute(
                    "SELECT ip AS target_ip FROM cves WHERE scan_id=? LIMIT 1", (scan_id,)
                ).fetchone()
            if row_target and row_target["target_ip"]:
                target = row_target["target_ip"]
        ...
        for r in raw_cves:
            ...
            if not r_dict.get("cwe_id") or not r_dict.get("references"):
                ...
                r_dict["cwe_id"] = r_dict.get("cwe_id") or (info.get("cwe_id") if info else "CWE-200")
                r_dict["cwe_name"] = r_dict.get("cwe_name") or (info.get("cwe_name") if info else "Exposure of Sensitive Information")
                ...
                r_dict["references"] = r_dict.get("references") or (info.get("references") if info else default_ref)
            rows.append(r_dict)
    ...
    def risk_report():
        ...
        if scan_id and not target_ip:
            r_row = conn.execute("SELECT ip FROM risk_summary WHERE scan_id=? LIMIT 1", (scan_id,)).fetchone()
            if not r_row:
                r_row = conn.execute("SELECT target_ip AS ip FROM host_status WHERE scan_id=? LIMIT 1", (scan_id,)).fetchone()
            if r_row and r_row["ip"]:
                target_ip = r_row["ip"]
        ...
        # 1. Available Targets List (strictly isolated to current user)
        available_targets = []
        try:
            if current_user_id is not None:
                target_rows = conn.execute(
                    ...
                    SELECT target_ip 
                    FROM host_status 
                    WHERE target_ip IS NOT NULL AND target_ip != '' AND user_id = ?
                    GROUP BY target_ip
