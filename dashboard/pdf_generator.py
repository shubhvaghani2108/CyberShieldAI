...
def _report_styles():
    ...
    styles.add(
        ParagraphStyle(
            name="RSection", fontName="Helvetica-Bold", fontSize=12, leading=15,
            textColor=REPORT_INK, spaceBefore=11, spaceAfter=4,
        )
    )
    styles.add(
        ParagraphStyle(
            name="RCell", fontName="Helvetica", fontSize=8.5, leading=11,
            textColor=REPORT_INK,
        )
    )
    styles.add(
        ParagraphStyle(
            name="RCellMuted", fontName="Helvetica", fontSize=8.5, leading=11,
            textColor=REPORT_MUTED,
        )
    )
    styles.add(
        ParagraphStyle(
            name="RBullet", fontName="Helvetica", fontSize=9, leading=13,
            textColor=REPORT_INK, leftIndent=12, bulletIndent=0,
        )
    )
    styles.add(
        ParagraphStyle(
            name="REmpty", fontName="Helvetica-Oblique", fontSize=9,
            textColor=REPORT_MUTED,
        )
    )
    return styles
...
def _report_data_table(header, rows, col_widths, styles, risk_col=None):
    """Builds a styled reportlab Table with dark navy header and clean rows."""
    body_style = styles["RCell"]
    table_rows = [[Paragraph(f"<b>{h}</b>", ParagraphStyle(
        name=f"Head_{h}", fontName="Helvetica-Bold", fontSize=8.5, leading=11,
...
def _findings_sections(flow, styles, findings, page_width):
    ...
    ports, services = findings.get("ports") or [], findings.get("services") or []
    vulnerabilities, cves = findings.get("vulnerabilities") or [], findings.get("cves") or []
    os_info, risk = findings.get("os_info"), findings.get("risk")

    flow.append(Paragraph("Operating System &amp; Risk Score", styles["RSection"]))
    os_name = os_info["os_name"] if os_info and "os_name" in os_info and os_info["os_name"] else "Not detected"
    os_details = os_info["os_details"] if os_info and "os_details" in os_info and os_info["os_details"] else "-"
    device_type = os_info["device_type"] if os_info and "device_type" in os_info and os_info["device_type"] else "-"
    risk_pairs = [
    ...
    if risk:
        risk_pairs += [
            ("Risk Level", risk["risk_level"] if "risk_level" in risk else "Low"),
            ("Risk Score", risk["total_score"] if "total_score" in risk else 0),
            ("Critical / High / Medium / Low",
             f"{risk.get('critical_count', 0)} / {risk.get('high_count', 0)} / {risk.get('medium_count', 0)} / {risk.get('low_count', 0)}"),
        ]
    ...
    if vulnerabilities:
        ...
        for v in vulnerabilities:
            ...
            rows.append([f"{v_dict['port']}/{v_dict.get('service') or '-'}", v_dict.get("risk", "Medium"), str(cvss), vector[:22], ac, pr, imp[:25], rec[:30]])
        flow.append(_report_data_table(
    ...
    if cves:
        ...
        for cv in cves:
            ...
            rows.append([cv_dict.get("cve_id", "-"), cwe, f"{cv_dict.get('port', '-')}/{cv_dict.get('service') or '-'}", cv_dict.get("severity", "Medium"), str(cvss), published, exploit])
        flow.append(_report_data_table(
...
def _build_ip_scan_pdf(user_id=None):
    ...
    if not latest_ip:
        ...
        overview = [
            ("Target IP", latest_ip),
            ("Host Status", host["status"] if host and "status" in host else "Unknown"),
            ("Scan Time", host["scan_time"] if host and "scan_time" in host else "-"),
        ]
        ...
        findings = {
            "ports": ctx.get("ports_data") or [],
            "services": ctx.get("services") or [],
            "vulnerabilities": ctx.get("vulnerabilities_data") or [],
            "cves": ctx.get("cves_data") or [],
            "os_info": _OSShim(os_dict) if os_dict and isinstance(os_dict, dict) else os_dict,
            "risk": ctx.get("risk"),
        }
...
def _build_url_scan_pdf(user_id=None):
    ...
    doc = SimpleDocTemplate(
        pdf_path, pagesize=A4,
        leftMargin=36, rightMargin=36, topMargin=26, bottomMargin=26,
    ...
    if not url_scan:
        ...
        overview = [
            ...
            ("Domain", url_scan.get("domain") or "-"),
            ("Resolved IP", url_scan.get("ip") or "Unknown"),
            ("Protocol", str(url_scan.get("protocol")).upper() if url_scan.get("protocol") else "-"),
            ("Threat Score", f"{url_scan.get('score', 0)} / 100"),
            ("Risk Level", url_scan.get("risk", "Low")),
            ("Scan Time", url_scan.get("scan_time", "-")),
        ]
        flow.append(Paragraph("Target Overview", styles["RSection"]))
        flow.append(_report_kv_table(overview, styles, [page_width * 0.35, page_width * 0.65]))
        ...
        if url_ctx.get("url_tech_server") or url_ctx.get("url_tech_list"):
        ...
        if intel:
            ...
            if url_scan.get("ip") in ("Unknown", "unknown", None) or url_scan.get("protocol") == "unknown":
        ...
        if url_scan.get("ip") and url_scan.get("ip") != "Unknown":
