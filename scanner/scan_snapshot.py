...
def get_scan_snapshot(scan_id: str) -> dict:
    ...
    Caches immutable completed snapshots to eliminate 9 redundant database queries on repeated views.
    """
    if not scan_id:
        return None

    if scan_id in _SNAPSHOT_CACHE:
        return dict(_SNAPSHOT_CACHE[scan_id])
    ...
    try:
        ...
        # Cache completed snapshots
        if scan_id and (url or ip or open_ports or vulnerabilities):
            if len(_SNAPSHOT_CACHE) >= _SNAPSHOT_CACHE_MAX:
                _SNAPSHOT_CACHE.pop(next(iter(_SNAPSHOT_CACHE)), None)
            _SNAPSHOT_CACHE[scan_id] = snapshot
